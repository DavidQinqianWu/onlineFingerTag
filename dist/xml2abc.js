//~ Revision: 116, Copyright (C) 2014-2017: Willem Vree
...
(function () {
  ...
  Music.prototype.appendNote = function (v, note, noot) {
    ...
    // YYMCode: 因为添加了 id 所以肯定不是noot ,必须是noot[0];
    if (noot[0] != "z" && noot[0] != "x") {
      /// *******************/
  ...
  Music.prototype.outVoices = function (divs, ip) {
    ...
    for (iv in this.vnums) {
      // yymcode : 修改, 一个voice没有音,也要生成, 针对的是 单手谱子
      // YYMComment: 如果是无效 voice, 就不生对应的字符串
  ...
  ABCoutput.prototype.mkHeader = function (stfmap, partlist, midimap, vmpdct, koppen) {
  ...
  function outVoice(measure, divs, im, ip, unitL) {
    ...
    while (vs.indexOf("!15ma(!!15ma)!") >= 0) vs = vs.replace(/!15ma\(!!15ma\)!/g, ""); // remove empty ottava's
    return vs;
  }
  function sortMeasure(voice, m) {
    ...
    if (time == 0)
      infof("empty measure in part %d, measure %d, it should contain at least a rest to advance the time!", [
        m.ixp + 1,
        m.ixm + 1,
      ]);
    return v;
  ...
  Parser.prototype.matchSlur = function (type2, n, v2, note2, grace, stopgrace) {
    ...
    if (n in this.slurBuf) {
      ...
      if (type2 != type1) {
        ...
        infof("double slur numbers %s-%s in part %d, measure %d, voice %d note %s, first discarded", [
          type2,
          n,
          this.msr.ixp + 1,
          this.msr.ixm + 1,
  ...
  Parser.prototype.doNotations = function (note, $nttn, isTab) {
    ...
    if (!isTab)
      $fingering.each(function () {
        ...
        // YYMCode: 如果出现了一个由多个指法, 就删除剩余指法, 而且需要删除掉空格换行
        let text = $(this)
          .text()
          .replace(/[^0-9]/gi, "");
        let temp = parseInt(text[0]);
        if (!Number.isNaN(temp)) {
          note.before.push("!" + temp + "!"); // fingering goes before chord (addChord)
        }
      });
    ...
    if (glis.length) {
      ...
      if (glis.attr("type") == "start") note.before.unshift(format("!%s(!", [lt]));
  ...
  //YYMComment: 生成note abc
  // YYMCode: 里面添加了 note的id 由 I...J 组成, 替换了整个doNote方法
  Parser.prototype.doNote = function ($n) {
    var note = new Note(0, null);
    note.noteId = curNoteId++;
    ...
    if (
      ...
      noot += `I${note.noteId}J-`;
    } else {
      noot += noot === "z" || noot === "x" ? "" : `I${note.noteId}J`;
    }
    xs = $n
      .find("beam")
      .map(function () {
  ...
  Parser.prototype.doAttr = function ($e) {
    ...
    if (fifths) {
      ...
      else if (key != abcOut.key || !first) this.msr.attr += "[K:" + key + "]"; // otherwise -> voice
    }
  ...
  Parser.prototype.doDirection = function ($e, i, $es) {
    ...
    function startStop(dit, dtype, vs, stfnum) {
      ...
      // ******************/
      if (t.attr("size") === "15") {
        typmap = { down: "!15ma(!", up: "!15mb(!", crescendo: "!<(!", diminuendo: "!>(!", start: "!ped!" };
      }
      // ******************/
      else {
      ...
      if (type in typmap) {
        ...
        if (k in dit.dirStk) {
          ...
          if (sk.type == "stop") addDirection(dit, x, vs, sk.tijd, stfnum);
            ...
            infof("%s direction %s has no stop in part %d, measure %d, voice %d", [
              dtype,
              sk.type,
              dit.msr.ixp + 1,
              dit.msr.ixm + 1,
        ...
        if (k in dit.dirStk) {
          ...
          if (type == "stop") {
            infof("%s direction %s has double stop in part %d, measure %d, voice %d", [
              dtype,
              type,
              dit.msr.ixp + 1,
              dit.msr.ixm + 1,
    ...
    for (var ix = 0; ix < dirtyps.length; ++ix) {
      ...
      if (metr.length) {
        ...
        if (metr.find("beat-unit-dot").length) tempo_units = simplify(tempo_units[0] * 3, tempo_units[1] * 2);
        r = metr
          .find("per-minute")
          .text()
      ...
      for (key in dynamics_map) {
        ...
        if (dirtyp.find("dynamics>" + key).length) this.msc.appendElem(vs, val, 1); // to voice
      }
    ...
    if (tempo) {
      ...
      } else this.msc.appendElem(v1, format("[Q:%d/%d=%s]", [tempo_units[0], tempo_units[1], tempo])); // otherwise -> 1st voice
    }
  ...
  Parser.prototype.doPrint = function ($e) {
    ...
    if ($e.attr("new-system") == "yes" || $e.attr("new-page") == "yes") return this.nolbrk ? "" : "$"; // a line break
  };
  ...
  Parser.prototype.addMidiMap = function (ip, vvmap) {
    ...
    if (ks.length) defInstr = instr[ks[0]];
